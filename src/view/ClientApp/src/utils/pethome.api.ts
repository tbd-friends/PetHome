...
export class AnimalCommandsClient {
    ...
    updateAnimal(id: string, model: UpdateAnimalInputModel): Promise<void> {
        let url_ = this.baseUrl + "/animal/update/{id}";
        if (id === undefined || id === null)
            throw new Error("The parameter 'id' must be defined.");
        url_ = url_.replace("{id}", encodeURIComponent("" + id)); 
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processUpdateAnimal(_response);
        });
    }

    protected processUpdateAnimal(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export interface UpdateAnimalInputModel {
    species?: string | undefined;
    color?: string | undefined;
    breed?: string | undefined;
    gender?: string | undefined;
